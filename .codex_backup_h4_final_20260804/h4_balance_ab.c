...
static uint32_t h4_millis(void)
{
    return g_millis;
}
...
static bool vision_is_recent(uint32_t now)
{
    ...
    return (vision->good_frames > 0U) &&
        ((now - vision->last_valid_ms) <= H4_VISION_STALE_TIMEOUT_MS);
}

bool h4_balance_ab_vision_ready(void)
{
    return vision_is_recent(h4_millis());
}
...
{
    const h4_tuning_runtime_t *runtime = h4_tuning_link_runtime();
    float command_deg = clampf(desired_deg,
        -runtime->kick_angle_deg, runtime->kick_angle_deg);

    /* Emergency/terminal commands must reach the requested safe angle in one
     * frame. Normal control commands remain slew-limited. */
    ...
    if (!force &&
        (absf(command_deg - g_last_motor_deg) <
         H4_MOTOR_COMMAND_MIN_CHANGE_DEG) &&
        ((now - g_last_motor_command_ms) <
         H4_MOTOR_COMMAND_MAX_INTERVAL_MS)) {
        return true;
    }
...
static void stop_vehicle(uint32_t now, h4_state_t final_state,
                         const char *event)
{
    motor_coast();
    g_target_rpm = 0.0f;
    ...
    g_finish_elapsed_ms = (g_run_start_ms == 0U) ?
        0U : (now - g_run_start_ms);
    g_state = final_state;
...
static void reset_ready(uint32_t now)
{
    ...
    (void) send_motor_angle(now, 0.0f, true);
    g_state = vision_is_recent(now) ? H4_READY : H4_WAIT_VISION;
}
...
static void start_run(uint32_t now)
{
    ...
    g_centered_since_ms = 0U;
    g_state = H4_ACCEL;
    h4_tuning_link_event("RUN_STARTED");
}
...
static void request_precenter(uint32_t now)
{
    ...
    g_centered_since_ms = 0U;
    g_state = H4_READY;
    h4_tuning_link_event("PRECENTER_STARTED");
    (void) now;
}
                             ...
                             const h3_vision_sample_t *vision)
{
    ...
    if (!g_start_pending || (g_state != H4_READY)) {
        return;
    }
    centered = (absf(vision->position_mm - H4_TARGET_MM) <=
                H4_PRECENTER_WINDOW_MM) &&
               (absf(vision->velocity_mm_s) <=
                H4_PRECENTER_MAX_SPEED_MM_S);
    if (!centered) {
        g_centered_since_ms = 0U;
        return;
    }
    ...
    } else if ((now - g_centered_since_ms) >= H4_PRECENTER_STABLE_MS) {
        h4_tuning_link_event("PRECENTER_COMPLETE");
        start_run(now);
    }
}
...
static void handle_key(uint32_t now)
{
    ...
    if ((now - g_last_key_ms) < H4_KEY_DEBOUNCE_MS) {
        return;
    }
    g_last_key_ms = now;

    if ((g_state == H4_READY) || (g_state == H4_WAIT_VISION)) {
        if (vision_is_recent(now)) {
            request_precenter(now);
        }
    } else if ((g_state == H4_STOPPED) ||
               (g_state == H4_FAULT_VISION) ||
               (g_state == H4_FAULT_LINE) ||
               (g_state == H4_FAULT_TIMEOUT)) {
        reset_ready(now);
    }
}
    ...
    while (h4_tuning_link_take_command(&command)) {
        switch (command) {
            case H4_TUNE_COMMAND_ARM:
                ...
                g_last_motor_deg = 0.0f;
                (void) send_motor_angle(now, 0.0f, true);
                h4_tuning_link_event("ARMED");
                break;
            case H4_TUNE_COMMAND_START:
                if (!g_motor_armed) {
                    h4_tuning_link_event("REJECT_NOT_ARMED");
                } else if (!vision_is_recent(now)) {
                    h4_tuning_link_event("REJECT_NO_VISION");
                } else if ((g_state == H4_ACCEL) ||
                           (g_state == H4_CRUISE) ||
                           (g_state == H4_DECEL)) {
                    h4_tuning_link_event("REJECT_ALREADY_RUNNING");
                } else {
                    if (g_state != H4_READY) {
                        reset_ready(now);
                    }
            ...
            case H4_TUNE_COMMAND_RESET:
                reset_ready(now);
                h4_tuning_link_event("RESET_READY");
                break;
            case H4_TUNE_COMMAND_STOP:
                stop_vehicle(now, H4_STOPPED, "MANUAL_STOP");
                (void) send_motor_angle(now, 0.0f, true);
                break;
            default:
                break;
...
static void update_speed_profile(uint32_t now)
{
    ...
    if (g_state == H4_ACCEL) {
        ...
        if (u >= 1.0f) {
            ...
            g_planned_accel_m_s2 = 0.0f;
            h4_tuning_link_event("CRUISE_STARTED");
        } else {
    ...
    } else if (g_state == H4_CRUISE) {
        g_target_rpm = runtime->cruise_rpm;
        g_planned_accel_m_s2 = 0.0f;
        if (g_distance_mm >= runtime->decel_start_mm) {
            g_state = H4_DECEL;
            g_phase_start_ms = now;
            h4_tuning_link_event("B_PASSED_DECEL_STARTED");
        }
    } else if (g_state == H4_DECEL) {
        uint32_t phase_ms = now - g_phase_start_ms;
        float u = (float) phase_ms / (float) runtime->decel_time_ms;
        float rpm_per_s;

        if (u >= 1.0f) {
            stop_vehicle(now, H4_STOPPED, "RUN_FINISHED_REPOSITION_CAR");
            return;
        }
        g_target_rpm = runtime->cruise_rpm * (1.0f - smoothstep(u));
        rpm_per_s = -runtime->cruise_rpm *
            smoothstep_derivative(u) * 1000.0f /
            (float) runtime->decel_time_ms;
        g_planned_accel_m_s2 =
            rpm_per_s * circumference_m / 60.0f;
    }
}
...
static void run_line_control(uint32_t now)
{
    ...
    if (g_line.line_visible) {
        ...
        stop_vehicle(now, H4_FAULT_LINE, "FAULT_LINE_LOST");
        return;
    }

    update_speed_profile(now);
    if ((g_state != H4_ACCEL) && (g_state != H4_CRUISE) &&
        (g_state != H4_DECEL)) {
        return;
    }
    ...
    if (g_distance_mm >= H4_HARD_STOP_DISTANCE_MM) {
        stop_vehicle(now, H4_STOPPED, "HARD_DISTANCE_STOP");
    } else if ((now - g_run_start_ms) >= H4_RUN_FAILSAFE_MS) {
        stop_vehicle(now, H4_FAULT_TIMEOUT, "FAULT_TIMEOUT");
    }
}
                                  ...
                                  const h3_vision_sample_t *vision)
{
    ...
    if (g_kick_active) {
        bool moving_to_center =
            ...
            (absf(vision->velocity_mm_s) >=
             H4_KICK_EXIT_CENTER_SPEED_MM_S);
        if (((now - g_kick_start_ms) >= H4_KICK_MAX_MS) ||
            moving_to_center ||
            (absf(g_predicted_mm) < H4_STUCK_POSITION_MM)) {
            g_kick_active = false;
            g_kick_cooldown_until_ms = now + H4_KICK_COOLDOWN_MS;
            g_stuck_start_ms = 0U;
            h4_tuning_link_event("KICK_FINISHED");
        }
        return;
    }
    ...
    if (displaced_and_slow) {
        ...
        } else if ((now - g_stuck_start_ms) >= H4_STUCK_CONFIRM_MS) {
            g_kick_active = true;
            g_kick_start_ms = now;
            g_stuck_start_ms = 0U;
            h4_tuning_link_event("KICK_STARTED");
        }
    } else {
        g_stuck_start_ms = 0U;
    }
                             ...
                             const h3_vision_sample_t *vision)
{
    ...
    bool running = (g_state == H4_ACCEL) || (g_state == H4_CRUISE) ||
                   (g_state == H4_DECEL);

    /* Vehicle acceleration and braking need more tube authority than the
     * constant-speed balance loop.  Reuse the independently tunable kick
     * limit for those two short phases, while retaining the lower normal
     * limit in cruise so the ball loop does not become underdamped again. */
    if (g_start_pending || (g_state == H4_ACCEL) ||
        (g_state == H4_DECEL) ||
        (absf(g_predicted_mm) >= H4_LARGE_ERROR_BOOST_MM)) {
    ...
    if (absf(g_predicted_mm) <= H4_INTEGRAL_ACTIVE_ERROR_MM &&
        (dt_s > 0.0f) && (dt_s <= 0.2f)) {
        g_integral_mm_s += g_predicted_mm * dt_s;
        g_integral_mm_s = clampf(g_integral_mm_s,
            -H4_INTEGRAL_LIMIT_MM_S, H4_INTEGRAL_LIMIT_MM_S);
    } else {
    ...
    if (g_state == H4_ACCEL) {
        kd_deg_per_mm_s *= H4_ACCEL_KD_MULTIPLIER;
        uint32_t launch_ms = now - g_run_start_ms;
        if (launch_ms < H4_ACCEL_START_BIAS_HOLD_MS) {
            g_feedforward_deg += H4_ACCEL_START_BIAS_DEG;
        } else if (launch_ms < (H4_ACCEL_START_BIAS_HOLD_MS +
                               H4_ACCEL_START_BIAS_FADE_MS)) {
            float fade_u = (float) (launch_ms -
                H4_ACCEL_START_BIAS_HOLD_MS) /
                (float) H4_ACCEL_START_BIAS_FADE_MS;
            g_feedforward_deg += H4_ACCEL_START_BIAS_DEG *
    ...
    if (g_kick_active) {
        float kick_angle_deg = (g_state == H4_CRUISE) ?
            H4_CRUISE_KICK_ANGLE_DEG : runtime->kick_angle_deg;
...
{
    switch (g_state) {
        case H4_WAIT_VISION:  return "WAIT CAM";
        case H4_READY:        return "READY";
        case H4_ACCEL:        return "ACCEL";
        case H4_CRUISE:       return "CRUISE";
        case H4_DECEL:        return "DECEL";
        case H4_STOPPED:      return "STOP";
        case H4_FAULT_VISION: return "CAM LOST";
        case H4_FAULT_LINE:   return "LINE LOST";
        case H4_FAULT_TIMEOUT:return "TIMEOUT";
...
static void update_ui(uint32_t now)
{
    const h3_vision_sample_t *vision = h3_vision_get();
    char text[24];
    uint32_t elapsed = (g_run_start_ms == 0U) ?
        g_finish_elapsed_ms : (now - g_run_start_ms);

    ssd1306_clear();
    ssd1306_draw_text(0U, 0U, "H4 AB BALANCE");
    ...
    (void) snprintf(text, sizeof(text), "LINE%02X E%+4d",
        g_line.black_mask, (int) g_line.error);
...
void h4_balance_ab_process(void)
{
    ...
    if ((g_state == H4_WAIT_VISION) && vision_is_recent(now)) {
        g_state = H4_READY;
        h4_tuning_link_event("VISION_READY");
    }
    handle_key(now);
    if (g_start_pending && !vision_is_recent(now)) {
        ...
        g_state = H4_WAIT_VISION;
        (void) send_motor_angle(now, 0.0f, true);
        h4_tuning_link_event("PRECENTER_VISION_LOST");
    }
    running = (g_state == H4_ACCEL) || (g_state == H4_CRUISE) ||
              (g_state == H4_DECEL);

    if (running && !vision_is_recent(now)) {
        stop_vehicle(now, H4_FAULT_VISION, "FAULT_VISION_LOST");
        (void) send_motor_angle(now, 0.0f, true);
    ...
    if (running &&
        ((now - g_last_line_control_ms) >= H4_LINE_CONTROL_PERIOD_MS)) {
        g_last_line_control_ms = now;
        run_line_control(now);
    }
    if (running &&
        ((now - g_last_speed_control_ms) >= H4_SPEED_CONTROL_PERIOD_MS)) {
        g_last_speed_control_ms = now;
    ...
    h4_tuning_link_telemetry(now, (uint8_t) g_state, g_motor_armed,
        ...
        g_kick_active,
        (g_run_start_ms == 0U) ? 0U :
            (((g_state == H4_STOPPED) || (g_state == H4_FAULT_VISION) ||
              (g_state == H4_FAULT_LINE) ||
              (g_state == H4_FAULT_TIMEOUT)) ?
             g_finish_elapsed_ms : (now - g_run_start_ms)));

    if (g_oled_present && ((now - g_last_ui_ms) >= H4_UI_UPDATE_MS)) {
        g_last_ui_ms = now;
        update_ui(now);
    }
    if (g_oled_present &&
        ((now - g_last_oled_page_ms) >= H4_OLED_PAGE_UPDATE_MS)) {
